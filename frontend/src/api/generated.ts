...
export interface paths {
    ...
    "/api/event-types/{id}/slots": {
        ...
        /** @description Get free booking slots for a given event type within the configured booking window. Slots are computed based on the event type duration, owner working hours (server config), and exclude any slots that overlap with existing bookings of any event type. All returned slots have duration equal to EventType.durationMinutes. Slots are sorted by startUtc ascending. If from or to are not provided, defaults to [now, startOfCurrentHostDay + bookingWindowDays days), and extends by one more day when today has no free slots for the selected event type. Returns 400 if from >= to or values are outside the booking window. Returns 404 if eventTypeId does not exist. */
